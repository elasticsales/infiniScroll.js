...
(function() {
  Backbone.InfiniScroll = function(collection, options) {
    ...
    self.options = _.defaults(options, {
      ...
      // 'Target' should be different than 'binder' especially in case where binder could get tall based on 
      // some content *other than* the content in the "target".
      target: $(window),
    ...
    self.onResize = _.debounce(function() {
      viewportIsFull = false;
      self.watchScroll();
    }, 300);
    ...
    self.onSync = function(obj, resp, options) {
      ...
      // Look for edge case where all fetched results (an entire page or more) fits
      // into the visible viewport, so no scrollbar is visible so no onscroll event will be fired,
      // but there are still more objects to be fetched/displayed.
      // This implementation assumes that original fetch happens after infiniScroll was initialized
      // (so it can listen for the 'sync' event).
      if (!viewportIsFull) self.watchScroll();
    };
